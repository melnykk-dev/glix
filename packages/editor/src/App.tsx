...
import { Move, RotateCcw, Maximize2, BarChart2, Monitor } from 'lucide-react';
                ...
                left={
                    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '4px 8px', borderTop: '1px solid var(--glix-border)', background: 'var(--glix-bg-deep)' }}>
                            <button
                                className={`icon-btn${showProfiler ? ' active' : ''}`}
                                style={{ fontSize: 9, gap: 4, display: 'flex', alignItems: 'center', color: showProfiler ? 'var(--glix-accent)' : 'var(--glix-text-dim)' }}
                        ...
                        {/* Profiler panel (collapsible) */}
                        {showProfiler && (
                            <div style={{ borderTop: '1px solid var(--glix-border)' }}>
                                <Profiler />
                            </div>
                        )}
                    </div>
                ...
                right={
                    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
                        <div className="tab-bar">
                            {(['inspector', 'logic', 'render', 'script', 'tilemap', 'project'] as const).map(tab => (
                                <button
                                    key={tab}
