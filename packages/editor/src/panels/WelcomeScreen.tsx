...
const TEMPLATES = [
    { id: 'empty', label: 'Empty project', desc: 'Blank scene, no assets' },
    { id: 'platformer', label: 'Platformer', desc: 'Player, physics, tilemap' },
    { id: 'topdown', label: 'Top-down', desc: 'Camera, movement, collision' },
];
...
export const WelcomeScreen: React.FC<Props> = ({ onClose }) => {
    ...
    const loadTemplate = async (id: string) => {
        try {
            const mod = await import(`../templates/${id}.glix`);
            setProject(mod.default);
            onClose();
        } catch {
            onClose();
        }
        ...
        <div className="dialog-overlay">
            <div style={{
                ...
                <div style={{ padding: '20px 28px 24px' }}>
                    {/* Templates */}
                    <div style={{ fontSize: 10, fontWeight: 700, color: 'var(--glix-text-muted)', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 10 }}>Start from a template</div>
